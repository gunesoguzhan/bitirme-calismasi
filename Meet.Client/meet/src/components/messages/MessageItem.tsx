...
export function MessageItem(props: MessageProps) {
  ...
  return (
    <div className={props.message.sender.id === authContext?.user?.id ? 'flex items-end justify-end' : 'flex items-start'}>
      <div className={props.message.sender.id === authContext?.user?.id ? 'bg-slate-800 rounded-lg px-4 py-2 max-w-[75%] break-all' : 'bg-[#190d30] rounded-lg px-4 py-2 max-w-[75%] break-all'}>
        <p className="text-sm">{props.message.messageText}</p>
        <div className="flex">
          <div className="text-xs text-gray-400 basis-full">{props.message.sender.id !== authContext?.user?.id ? `${props.message.sender.firstName} ${props.message.sender.lastName}` : ''}</div>
